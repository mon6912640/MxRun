...
- **系统托盘**：蓝色圆点图标，右键菜单（显示/隐藏、快捷项管理、新建条目、设置、退出），双击呼出
...
- **删除与快捷项管理**：`Delete` 删除（第一次问、第二次删）、`Insert` 新建、`Ctrl+D` 定位到文件、行右键菜单；托盘 → 快捷项管理 打开全部条目的管理视图（可筛选、按启动次数排序、双击编辑）。**删掉的条目不会被下一次导入搬回来**（墓碑）
