...
- **应用自动发现**：默认开启，后台增量扫描开始菜单（跳过卸载程序、帮助文档、网页快捷方式），**按真实程序路径去重**——手工加过的条目不会被覆盖；扫出来的东西在管理视图里按"自动发现"筛选、一键清理，**删过的不会再被扫回来**。全程在首帧之后的后台线程完成，冷启动与呼出延迟不受影响（实测首帧仍 256ms、扫描 180ms 后台、二次启动 0 次 COM 调用）
- **键盘流**（对齐原版）：**呼出后直接按数字**即可执行第 N 项（`1`..`9`，`0` = 第 10 项；一旦开始打字，数字就回到搜索框，所以 `python 3.12` 照常能打）；
  `Ctrl/Alt+数字`、`;`（第 2 项）、`'`（第 3 项）在任何状态都能用（行首有淡灰序号）；
  `Tab`/`Shift+Tab` 当上下、`↑↓` 首尾循环；`Insert` 新建、`Delete` 两步删除、`F2` 编辑、`Ctrl+D` 定位、`Ctrl+C` 复制命令行、`Ctrl+L` 最近列表。
  **单击选中、双击/中键才执行**（原版语义，避免误触）
- **窗口**：按住卡片空白处即可拖动，**位置会记住**（换显示器也会夹回屏幕内）；托盘**单击**切换显隐
