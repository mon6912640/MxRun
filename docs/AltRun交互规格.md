...
> **进度（2026-09-17）**：已经补上的有 —— **空格 = 执行当前项**、**参数机制**（形态按用户拍板改成顶框切换式，
> 不是弹框；见 `开发进度.md` §2.10）、**失焦立即隐藏**、**Esc 二级语义**、**`*` 前缀**、
> **单实例唤醒**（第二个实例静默交接后退出，见 §2.11）、**右键集成：SendTo + shell 菜单**（§2.11）、
> **无结果回车 → 添加它**（§2.11）、**F2 编辑**（§2.11）、
> **Insert 新建 / Delete 删除（两步确认）/ 快捷项管理 / Ctrl+D 定位 / 行右键菜单**（§2.13）、
> **序号 + Ctrl/Alt+数字 + `;`/`'`**（序号改成行首淡灰小数字，不上 `*1|` 前缀；
> 另外**空输入框时裸数字也能执行第 N 项**，这是用户追加的现代做法，原版没有）、
> **Tab/Shift+Tab、↑↓ 首尾循环**、**单击选中 + 双击/中键执行**、
> **窗口拖动 + 位置记忆**（WinTop/WinLeft）、**托盘单击**、**Ctrl+C 复制命令行 / Ctrl+L 最近列表**（§2.15）。
> **还没补的**：`Alt+S` 打开管理器 / `Alt+C` 打开配置这类原版窗体快捷键、
> `{%c}` 在热键按下瞬间取值（现在是执行瞬间）、`IndexFrom0to9`（序号从 0 起）这类可配置项。
